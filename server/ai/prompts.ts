...
Before you generate the image, look at the original photo and explicitly enumerate, silently to yourself, every repeated or countable element it actually contains: the number of stones (and where each one sits), the number of beads/balls/tassels/granulation points in any dangling or clustered group, the number of chain or mesh strands, the number of visible chain links across a representative stretch, and every distinct engraving, motif, or hallmark stamp present. Hold each of these exact numbers fixed as a hard constraint for the entire rest of this task - they are facts about a physical object, not stylistic choices. If you cannot clearly see or count an element from the photo provided, do not guess a number for it or invent detail to fill the gap - render that area only as clearly as the source photo actually supports.
...
Compare what you are about to output against the counts you took in STEP 1: same number of stones in the same positions, same number of beads/balls/tassels/granulation points, same number of chain/mesh strands, same engravings/motifs/hallmarks with nothing added and nothing smoothed away. This is the last and most common way this task fails - not the lighting, the background, or the pose, but a small count that quietly drifted during rendering. If anything does not match, fix it before producing the final image rather than after.
...
Respond ONLY as JSON matching this schema:
{
  ...
  "stoneCountMatches": boolean, // CRITICAL: does the stone count/position in enhancedCounts EXACTLY match originalCounts, with each stone in the same position, cut, and setting style? Fail if any stone was added, removed, split into a cluster, or re-cut. A single centre stone must never have become several smaller ones.
  "beadDetailPreserved": boolean, // CRITICAL: does the bead/ball/tassel/granulation count in enhancedCounts EXACTLY match originalCounts, at the same spacing? Fail if any were added or dropped, even if a different count would look more even.
  "chainPatternMatches": boolean, // CRITICAL: does the chain/strand count in enhancedCounts EXACTLY match originalCounts, with the same link shape and profile, consistent spacing, and the same clasp? Fail if strands were merged or split, or the link pattern changed. Automatically true if the piece has no chain or strand element at all.
  "engravingPreserved": boolean, // CRITICAL: does every engraving, motif, pattern, hallmark stamp, and surface texture in originalCounts still appear, unaltered and unsimplified, in enhancedCounts - and is there NOTHING in enhancedCounts that is absent from originalCounts? Fail in either direction: removing real detail and inventing new detail are both failures.
  "naturalDropPhysics": boolean, // If this piece has hanging chains, mesh, tassels, or ball/bead drops (e.g. jhumka, chandbali, bali, layered haars, charm bracelets): do they fall in smooth, symmetric, gravity-consistent curves - NOT tangled, kinked, flattened, pinched, or bent at an implausible angle? Does the drop count in enhancedCounts match originalCounts exactly? If the two earrings/sides of a pair are both visible, are their drops symmetric to each other? If the item has no hanging/repeated drop elements at all, this is automatically true.
