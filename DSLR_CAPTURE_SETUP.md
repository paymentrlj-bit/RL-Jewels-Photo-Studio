...
- **"Studio camera focus nudge failed", or the near/far arrows don't seem to do anything** — the four small arrow buttons next to Focus send `CMD=LiveView_Focus&value=<step>` (a negative step for near, positive for far; ±1 for the small arrows, ±5 for the double arrows) to fine-tune focus after autofocus gets close. Like `DoAutoFocus` above, this is **not yet confirmed** against this installation - it's digiCamControl's documented mechanism for its own focus-stacking feature, reused here. `bridge.log` lines for this are tagged `UNVERIFIED COMMAND focus nudge` with the exact value sent and digiCamControl's raw response - if the direction feels backwards (near arrow moves focus far, or vice versa), or the step size feels too big/small, that's a one-line fix (flip the sign, or adjust `NUDGE_STEP_SMALL`/`NUDGE_STEP_LARGE`) in `dslr-bridge/bridge.js`, not a deeper problem.
- **General note on "UNVERIFIED COMMAND" lines in bridge.log** — any digiCamControl command wired up without a manual browser test against this specific installation (unlike the step 6 endpoints, which were each confirmed working before being used in code) logs its raw response with that prefix, specifically so a wrong command/param can be spotted and corrected quickly from real usage instead of guessing blind.
