// Item-type suggestions, extracted verbatim from v1's src/types.ts.
// These are the store's own category names from its POS - not a generic
// jewellery taxonomy - so they stay exactly as they are.
//
// v1's STAFF_MEMBERS hardcoded list is deliberately NOT carried over:
// staff are real user accounts now, so the name on a product comes from
// who is signed in rather than a dropdown anyone can pick any entry from.
